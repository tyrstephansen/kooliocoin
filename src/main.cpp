...
#if defined(NDEBUG)
# error "Kooliocoin cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kooliocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kooliocoin Signed Message:\n";
...
{
    // Kooliocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if (nHeight<2) { nSubsidy =  231840 * COIN; }
     else if (nHeight<43200) { nSubsidy =  50 * COIN; }
     else if (nHeight<2102400) { nSubsidy = 10 * COIN; }
	else { nSubsidy = 1 * COIN; }
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 60; // Kooliocoin: 3.5 days
static const int64 nTargetSpacing = 60; // Kooliocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kooliocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kooliocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Kooliocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Kooliocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KooliocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KooliocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KooliocoinMiner(CWallet *pwallet)
{
    printf("KooliocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kooliocoin-miner");
    ...
    try { loop {
        ...
        printf("Running KooliocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KooliocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KooliocoinMiner, pwallet));
}
